  ...
  const getQuestionPapers = async () => {
    try {
      const response = await axios.get('http://localhost:8030/questions', {
        params: { department: "BioTechnology" }
      });
      ...
      <div className='p-4 max-w-7xl mx-auto'>
        ...
        <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6'>
          <div>
            ...
            <p className='text-gray-500'>Semester-wise question papers and resources</p>
          </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 overflow-y-auto max-h-96 p-2'>
              {filteredPapers.map((paper) => (
                <div key={paper._id || paper.id} className='border rounded-lg p-4 bg-white border-green-500 shadow-md hover:shadow-lg transition-shadow'>
                  <h3 className='text-lg font-semibold mb-2 text-green-800'>{paper.subject}</h3>
                  <p className='text-gray-600 text-sm'>Semester: {paper.semester}</p>
        ...
        <div className='p-6 rounded-lg shadow-md bg-[#e3f2fd] border border-blue-100'>
          ...
          <div className='my-4'>
            ...
            <input
              ...
              className='w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-blue-700 hover:file:bg-green-100 cursor-pointer'
            />
