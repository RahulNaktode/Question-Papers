...
function FirstYear() {
  ...
  const addQuestionPaper = async () => {
    ...
    const dataToSend = { ...formData, department: "FirstYear" };
  ...
  const getQuestionPapers = async () => {
    try {
      const response = await axios.get('http://localhost:8030/questions', {
        params: { department: "FirstYear" } // Fixed department param
      });
    ...
    <div className='bg-gray-50 min-h-screen'>
      ...
      <div className='p-4 max-w-7xl mx-auto'>
        ...
        <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6'>
          <div>
            <h1 className='text-2xl font-bold'>📃 First Year Engineering</h1>
            <p className='text-gray-500'>Foundation subjects, question papers and study materials</p>
          </div>
          ...
          {filteredPapers.length > 0 ? (
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 overflow-y-auto max-h-96 p-2'>
              {filteredPapers.map((paper) => (
                <div key={paper._id || paper.id} className='border rounded-lg p-4 bg-white border-green-500 shadow-md hover:shadow-lg transition-all'>
        ...
        <div className='p-6 rounded-xl shadow-md bg-[#e3f2fd] border border-blue-100'>
          <h2 className='mb-4 font-bold text-lg'>📤 Contribute First Year Resources</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <Input
              type="text"
              placeholder="Subject Name (e.g., Engineering Physics)"
