...
function ComputerScience() {
  ...
  const addQuestionPaper = async () => {
    ...
    const dataToSend = { ...formData, department: "ComputerScience" };
  ...
  const getQuestionPapers = async () => {
    try {
      const response = await axios.get('http://localhost:8030/questions', {
        params: { department: "ComputerScience" }
      });
      ...
      <div className='p-4 max-w-7xl mx-auto'>
        ...
        <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6'>
          <div>
            ...
            <p className='text-gray-500'>Access all semester-wise question papers and resources</p>
          </div>
        ...
        <div className='my-10'>
          ...
          {filteredPapers.length > 0 ? (
            ...
            <div className="bg-white p-10 rounded-lg border-2 border-dashed border-gray-200 text-center">
               <p className="text-gray-500 italic">No resources found for "{searchTerm}"</p>
            </div>
          )}
        </div>
        ...
        <div className='p-6 rounded-xl shadow-md bg-[#e3f2fd] border border-blue-100'>
          ...
          <div className='my-4'>
            <label className='block font-bold mb-2'>📸 Select Images</label>
            <input 
              type="file"
              ref={fileInputRef}
